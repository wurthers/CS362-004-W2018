...
int main (int argc, char* argv[]){
	...
	for (n = 0; n < numTests; n++){
		...
		if (trInDeck < 2){
			...
			// Test: Number of cards in hand after += - 1 + number of treasures in deck before
			printf("******* TEST %d: Number of cards in hand *******\n", totalTests);
			printf("Number of cards in hand after playing %s = %d; Target number = %d....", TESTCARD,
					post->handCount[testPlayer], pre->handCount[testPlayer] - 1 + trInDeck);
			passedTests += assert_(post->handCount[testPlayer] == pre->handCount[testPlayer] - 1 + trInDeck);
			totalTests += 1;
